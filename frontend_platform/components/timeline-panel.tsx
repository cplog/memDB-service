...
export function TimelinePanel({
  ...
  // Animate stream cards on data change
  useEffect(() => {
  ...
  return (
    <section className="flex flex-col flex-1 min-h-0">
      <div className="flex flex-wrap items-center gap-x-3 gap-y-2 px-4 py-1.5 border-b bg-[hsl(var(--card))] sm:px-5">
      ...
      <ScrollArea className="flex-1 min-h-0 bg-[hsl(var(--canvas))]">
        {loading ? (
          ...
          <div className="w-full max-w-none px-4 py-4 sm:px-6 space-y-5">
            {/* Activity bar chart — full canvas width */}
            {buckets.length > 0 ? (
              <div className="space-y-2 w-full" ref={barsRef}>
                ...
                <div className="flex flex-wrap gap-3 text-[10px] text-[hsl(var(--vault-muted))]">
              ...
              <div ref={streamRef}>
                {groupedStream.map(([monthKey, mems]) => (
                  <div key={monthKey} className="mb-6">
                    <h3 className="text-xs font-semibold uppercase tracking-wide text-[hsl(var(--vault-muted))] mb-3 pl-[calc(4.5rem+1.75rem)] sm:pl-[calc(5rem+2rem)]">
                    ...
                    <div className="relative">
                      {mems.map((mem, idx) => {
                        ...
                        return (
                          <div
                            key={mem.id}
                            className="stream-card grid grid-cols-[4.5rem_12px_minmax(0,1fr)] sm:grid-cols-[5rem_12px_minmax(0,1fr)] gap-x-3 sm:gap-x-4"
                          >
                            ...
                            <article className="min-w-0 pb-5 border-b border-[hsl(var(--border))] last:border-b-0">
